...
void AM::Init()
{
    ...
    am.hover_cursorTexture = std::make_unique<Texture>("src/img/hover_cursor.png");
    GLFWimage image;
    image.width = 16;
    image.height = 16;
    image.pixels = new unsigned char[image.width * image.height * 4];
    memcpy(image.pixels, am.def_cursorTexture->GetPixels(), image.width * image.height * 4);
    ...
    memcpy(image2.pixels, am.hover_cursorTexture->GetPixels(), image.width * image.height * 4);
    ...
    am.modelInstancedShader = std::make_unique<Shader>("src/graphics/shaders/instanced_model_3d/vert.v",
...
std::unique_ptr<Texture> loadFontAtlas(int fSize, enum AM::FontSize fontSizeEnum)
{
    ...
    for (char c = '!'; c <= '~'; ++c) {
        ...
        AM::atlasInfo[fontSizeEnum][(unsigned int)c].yPos =
            gs->bitmap_top - gs->bitmap.rows;  // this should be negative // - gs->bitmap_top;
