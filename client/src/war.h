...
class War {
   ...
   public:
    ...
    void AttackerSiege(int s)
    {
        attWarScFromSiege += s;
    }

    void AddAttacker(std::string a, int id)
    {
        attackers.push_back(a);
        attackersIds.push_back(id);
    }
    void AddDefender(std::string d, int id)
    {
        defenders.push_back(d);
        defendersIds.push_back(id);
    }
    ...
    int GetAttackerWarScore() const
    {
        return attackerWarScore + attWarScFromSiege;
    }
    void SetWarScore(int score)
    {
        attackerWarScore = score;
    }

    int GetAttacker()
    {
        if (attackersIds.size())
            return attackersIds[0];
        else
            return -1;
    }
    int GetDefender()
    {
        if (defendersIds.size())
            return defendersIds[0];
        else
            return -1;
    }

    std::vector<std::string> GetAttackers() const
    {
        return attackers;
    }
    std::vector<std::string> GetDefenders() const
    {
        return defenders;
    }
