...
void HourlyUpdate(sf::Packet& packet, std::vector<std::shared_ptr<Unit>>& units,
                  std::vector<std::unique_ptr<Battle>>& battles, float mapChunkScale, const unsigned char* height, int mapWidth);
...
void NewBattle(sf::Packet& packet, std::vector<std::shared_ptr<Unit>>& units,
...
void NewUnitInBattle(sf::Packet& packet, std::vector<std::shared_ptr<Unit>>& units,
