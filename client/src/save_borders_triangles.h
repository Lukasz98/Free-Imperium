...
std::vector<BorderVertexLoc> saveBordersTriangles(int mapWidth, int mapHeight, int scale, const unsigned char* h)
{
    ...
    std::fstream file;
    file.open("generated_data/BordersData.txt", std::fstream::in);
    std::string ss;
    while (file >> ss) {
        ...
        borVerts.push_back(BorderVertexLoc{.pos = Vec3{((float)x1), ((float)y1), (float)h[i1]},
                                        .tc = Vec2{(float)x1 / mapWidth, (float)y1 / mapHeight}});
        borVerts.push_back(BorderVertexLoc{.pos = Vec3{((float)x2), ((float)y2), (float)h[i2]},
    ...
    bool* visited = (bool*)calloc(borVerts.size() / 2, sizeof(bool));// = {0};
    ...
    file.open("generated_data/BordersData2.txt", std::fstream::out);
