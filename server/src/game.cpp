    ...
    : clients(clients), countries(countries)
{
    ...
    std::sort(provinces.begin(), provinces.end(), [](const std::shared_ptr<Province> & c1, const std::shared_ptr<Province> & c2){
        return c1->GetId() < c2->GetId();
    });
    ...
    if (c->GetGold() > 1.0f && clearIncome > 0.3f) {
        ...
        for (auto & prov : provs) {
            if (prov->GetManpower() > 0 && prov->GetSiegeLevel() == 0) {
                ...
                auto unit = std::make_shared<Unit>(prov->GetName() + " unit", prov->GetUnitPos(), temp, c->GetName(), prov->GetId(), c->GetId(), prov->GetId());
...
LogFunkStart;
    ...
    for (auto & prov : cProvs) {
        ...
        for (auto n : neighb) {
            ...
            if (nProvCountry->GetRel(c->GetId()) <= 0 && c->GetRel(nProvCountry->GetId()) <= 0 && std::rand() % 10 == 0) {
...
LogFunkStart;
    if (lastDay != 1) // monthly update
        return;
    auto warIt = std::find_if(wars.begin(), wars.end(), [cId = c->GetId()](War * war) { return (war->IsAttacker(cId) || war->IsDefender(cId)); });
...
LogFunkStart;
    for (auto w : wars) {
        ...
        if (myWarScore > 20) {
            ...
            if (warIt == wars.end())
                continue;
            DoTheThing::SendPeace(wars, provinces, countries, peaceOffer, offeredProvinces, toSend, recipant, warIt, date, offersForHumans);
        }
    }
LogFunkEnd;
}
                ...
                for (auto & uu : units) {
                    ...
                    for (auto war : wars) {
                        ...
                        battle.AddDefender(uu); // jest losowe, ale bedzie teraz mozna latwo to poprawic odkad
                                                // provincja trzyma ref do unitow. Wystarczt sprawdzac kto pierwszy wchodzi na prowincje
...
LogFunkStart;
    ...
    if (country->GetGold() >= unitSize / 100) {
        ...
        auto unit = std::make_shared<Unit>(prov->GetName() + " unit", prov->GetUnitPos(), unitSize, country->GetName(), provId, countryId, provId);
